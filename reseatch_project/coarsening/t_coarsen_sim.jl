...
@sync @distributed for set in sets
    all_data = h5read(
        "../single_component/data_inputs/student_t/1d/single_t_1d-dof=2-set-$set.jld",
        "data"
    )
    ...
    for alpha in alphas
        for n in ns
            ...
            save_fullpath = "./comp_outputs/student_t/1d/k_posterior-single_t_1d-coarsen=$alpha-dof=2-n=$n-set-$set.jld"
            save(save_fullpath, "k_posterior", k_posterior)
            # save_fullpath = "./comp_outputs/student_t/1d/z_draws-single_skew_normal_1d-coarsen=$alpha-alpha=7-n=$n-set-$set.jld"
            # save(save_fullpath, "z_draws",)
            println(Dates.now())
        end
    end
end
