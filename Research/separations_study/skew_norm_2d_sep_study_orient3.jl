...
@sync @distributed for set in sets
    for sep in seps
        ...
        for n in ns
            ...
            println(Dates.now())
            save(
                "./comp_outputs/skew_norm/2d/k_posterior-skew_normal_2d-alpha=$alpha-sep=$sep-n=$n-orient=3-set-$set.jld",
                "k_posterior",
                k_posterior
            )
            save(
                "./comp_outputs/skew_norm/2d/t_posterior-skew_normal_2d-alpha=$alpha-sep=$sep-n=$n-orient=3-set-$set.jld",
                "t_posterior",
                t_posterior
            )
        end
