...
for (i_n, n) in enumerate(ns)
    for (i_sep, sep) in enumerate(seps)
        ...
        all_data = h5read("./data_inputs/gaussian_mixtures_1d-sep=$sep.jld", "gaussian_data")
        ...
        save(
            "./comp_outputs/t_posteriors-mvn-dpm-sep=$sep-n=$n-1d_gaussian_mixtures.jld",
            "t_posteriors",
            t_posteriors
        )

        save(
            "./comp_outputs/k_posteriors-mvn-mfm-sep=$sep-n=$n-1d_gaussian_mixtures.jld",
            "k_posteriors",
            k_posteriors
        )
