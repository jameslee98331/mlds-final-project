...
# p = do.call("grid.arrange", c(plot_created_data, ncol = 5))
...
for (dof in dofs) {
  ...
  p = df %>% ggplot(aes(x = x)) +
    geom_histogram(bins = 20, fill = "#619cff", color = "#000000", alpha = 0.9) + 
    ggtitle(glue("dof={dof} - set={set}")) +
    theme(plot.title = element_text(size = 10))
  print(p)
  ggsave(
    glue("./plots/single_t_1d-dof={dof}_examples.pdf"),
    p,
    width = 5,
    height = 5,
    dpi = 600,
...
colnames(df) = c("iter", "t")
df %>% ggplot() + geom_point(aes(x = iter, y = t), size = 0.1)
```
...
for (n in ns) {
  ...
  summary_clustering_plot = df %>% ggplot(aes(x = x, y = y)) +
    ...
    ggtitle(glue("Summary Clustering (Best VI) - n={n}"))
